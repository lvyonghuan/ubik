...
public class Graph
{
    private static Dictionary<int, RuntimeNode>
        _runtimeNodes = new Dictionary<int, RuntimeNode>(); //运行时节点, key为节点ID，value为节点
    ...
    public class RuntimeNode
    {
        ...
        public void InitRuntimeNode(Dictionary<string, List<ConsumerBuffer>> consumerBuffers,
            Dictionary<string, ConsumerBuffer> consumerBuffer)
        {
            SetCommunicator();
        ...
        private void SetBuffer(Dictionary<string, List<ConsumerBuffer>> consumerBuffers,
            Dictionary<string, ConsumerBuffer> consumerBuffer)
        {
    ...
    public static void RemoveNode(int nodeId)
    {
        ...
        foreach (var input in node.Points.Input)
        {
            ...
            {
                ExecuteDeleteEdge(GetRuntimeNode(input.Value.NodeId), node, input.Value.PointToPointName, input.Key);
            }
        }
        ...
        foreach (var output in node.Points.Output)
        {
            for (int i = output.Value.Count - 1; i >= 0; i--)
            {
                var edge = output.Value[i];
                ExecuteDeleteEdge(node, GetRuntimeNode(edge.NodeId), output.Key, edge.PointToPointName);
            }
        }
    ...
    // 4.出点A与入点C建立连接。
    public static void UpdateEdge(int producerNodeId, int consumerNodeId, string producerNodePointName,
        string consumerNodePointName)
    {
        var producerNode = GetRuntimeNode(producerNodeId);
        var consumerNode = GetRuntimeNode(consumerNodeId);


        Core.Logger.Debug("Update edge from node " + producerNodeId + " to node " + consumerNodeId + " with point " +
                          producerNodePointName);

        //检查合法性,获取连接对象
        CheckUpdateValid(producerNode, consumerNode, producerNodePointName, consumerNodePointName,
            out var producerNodePointValue, out var consumerNodePointValue, out var hasConsumerNodeLinkedOtherNode);
        ...
        try
        {
            ...
            producerNode.Points.Output[producerNodePointName].Add(
                new Edge(consumerNodeId, producerNodePointValue, consumerNodePointName) { Buffer = consumerBuffer });
            ...
            //更新消费者节点的入点，指向生产者节点
            consumerNode.Points.Input[consumerNodePointName] =
                new Edge(producerNodeId, consumerNodePointValue, producerNodePointName) { Buffer = consumerBuffer };
        }
        finally
        {
            GraphLock.ExitWriteLock();
    ...
    //检查更新边合法性
    private static void CheckUpdateValid(RuntimeNode producerNode, RuntimeNode consumerNode,
        string producerNodePointName, string consumerNodePointName, out Value producerNodePointValue,
        out Value consumerNodePointValue, out bool hasConsumerNodeLinkedOtherNode)
    {
        //读锁
        ...
        try
        {
            ...
            //检查属性是否一致
            if (producerNode.Node.Output.First(e => e.Name == producerNodePointName).Attribute !=
                consumerNode.Node.Input.First(e => e.Name == consumerNodePointName).Attribute)
            {
                throw new UbikException("Output point " + producerNodePointName + " and input point " +
                                        consumerNodePointName + " attribute not match");
            }
        }
        finally
        {
    ...
    public static void DeleteEdge(int producerNodeId, int consumerNodeId, string producerNodePointName,
        string consumerNodePointName)
    {
        Core.Logger.Debug("Delete edge from node " + producerNodeId + " to node " + consumerNodeId);

        CheckDeleteEdge(producerNodeId, consumerNodeId, producerNodePointName, consumerNodePointName,
            out var producerNode, out var consumerNode);
        
        ExecuteDeleteEdge(producerNode, consumerNode, producerNodePointName, consumerNodePointName);

        Core.Logger.Debug("Delete edge from node " + producerNodeId + " to node " + consumerNodeId + " success");
    }

    private static void ExecuteDeleteEdge(RuntimeNode producerNode,RuntimeNode consumerNode,string producerNodePointName,string consumerNodePointName)
    {
        GraphLock.EnterWriteLock();
        try
        {
            //删除生产者节点中的边
            producerNode.Points.Output[producerNodePointName].RemoveAll(e => e.NodeId == consumerNode.Id);
    ...
    private static void CheckDeleteEdge(int producerNodeId, int consumerNodeId, string producerNodePointName,
        string consumerNodePointName, out RuntimeNode producerNode, out RuntimeNode consumerNode)
    {
        //读锁
        GraphLock.EnterReadLock();
        try
        {
            ...
            {
                throw new UbikException("Output point " + producerNodePointName + " not exist in node " +
                                        producerNodeTmp.Node.Name);
            }
